...
@Repository
@Transactional
public class DonationCampVolunteerDao {
...
public List<DonationCampVolunteer> getAllByCampID(int campID) {    
	  List<DonationCampVolunteer> volunteers = (List<DonationCampVolunteer>) getSession().getNamedQuery("DonationCampVolunteer.findByCampID").setInteger("campID", campID).list();
	return volunteers;
  }
  
  @SuppressWarnings("unchecked")
public List<DonationCampVolunteer> getAllByUserID(int userID) {    
	  List<DonationCampVolunteer> volunteers = (List<DonationCampVolunteer>) getSession().getNamedQuery("DonationCampVolunteer.findByUserID").setInteger("userID", userID).list();
return volunteers;
  }
